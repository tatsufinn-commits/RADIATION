...
## v3.10.7 — 2026-09-15 — RELEASE-TRUTH-GATE (5824)

**Release Truth Gate per acceptance fbce71b and greenlight RADIATION_PUBLIC_TREE_REPAIR_ACCEPTANCE_AND_RELEASE_TRUTH_GATE_GREENLIGHT_2026-09-15.md.** Base `fbce71bbfe4c62da71e6b347d36617474f840297` first green after 5810-5822 deletion failures (stale absent, 14 records, 0 findings, 24/24 vectors, 42·39·3·0, 25 OK, strict PASS, Actions validate+apply-report success). Purpose: prevent failure class where package/local worktree says deletion/repair occurred while pushed public commit contains only doc claims or extracted payload. Deliverables (non-boot, concise, no model records/sources/policy conclusions/profile assessments/runtime tools/provider calls/evaluations/routing/deployment/legal): 1) machine-readable release expectation `docs/RELEASE_TRUTH_GATE/EXPECTATION.json` pins immutable base SHA fbce71b, allowed/expected A/M/D paths incl required deletions, required generated artifacts (INVENTORY.generated.txt), mandatory validations with expected exit/status, risks list (wrong root, stale base, ZIP extraction without deletion, generated-file drift, doc claiming unverified CI, host/provider/surface confusion, unavailable credentials/remote verification). 2) repository-owned checker `scripts/release_truth_check.py` fails when base SHA wrong/not ancestor, required deletion still present, expected delete missing from diff, undeclared path included, generated artifact stale, DoD incomplete; self-test 7 vectors (6 negative: wrong base, file still on disk, delete missing from diff, undeclared path, stale artifact, missing DoD outcome → reject; 1 positive clean → pass). 3) DoD/risk-register template `docs/RELEASE_TRUTH_GATE/DoD_TEMPLATE.md` with pre-push checklist and risk register. 4) post-push receipt format `docs/RELEASE_TRUTH_GATE/POST_PUSH_RECEIPT_TEMPLATE.md` with remote SHA, fresh-clone commands/results, Actions URLs + conclusions. Scope locks: non-boot concise, no new model records/sources/policy/profile/runtime/provider/evaluation/routing/deployment/legal, no automatic git push/canonical apply/provider invocation, Commander authority unchanged, no claim local ZIP/SHA/expected CI proves public delivery. DoD before push: checker clean, negative vectors green, catalog 24/24, validator 42·39·3·0, unit 25 OK, strict no FAIL-class, whitespace clean. After Commander push: new clone of remote SHA with release-truth checker, catalog checker, validator, strict apply clean, plus success public Actions validate and apply-report URLs — only then may release docs describe delivery as public/green. Next after gate proven: surface-specific five-provider analysis using dated primary sources; explicit observed Arena/PASS activation study; then Commander-authorized local evaluations. Maintains distinction provider declarations vs host/session observations vs experimentally observed performance. Tool registry updated 22→23 tools, CAPABILITIES rendered via render_docs.py.
