# APPLY.ps1 - RADIATION patch 4400 "Reconciliation" (superset of 4200+4300) - seal transaction
...
Say "RADIATION patch 4400 - Reconciliation (gate: 4100 OK - supersedes 4200+4300)"
...
if ((Get-ChildItem "evidence/tasks/TID-2026-09-14-h/commands" | Measure-Object).Count -ne 6) { Die "bundle commands incomplete" }
Say "OK archive (15) + canonical bundle (6 commands) complete"
...
Remove-Item "APPLY.sh" -Force -ErrorAction SilentlyContinue   # cross-remove: BOTH runners retire (F-02)
Remove-Item $MyInvocation.MyCommand.Path -Force -ErrorAction SilentlyContinue  # unlink self BEFORE validation
if ((Get-Content "README.md" -Raw) -notmatch "v2\.7\.0") { Die "version not v2.7.0" }
...
Say "  Expected fresh-clone: 34 checks - 33 pass - 1 warn - 0 FAIL."
Say "===================================================="
