# APPLY.ps1 - RADIATION patch 4600 "Binding" (superset of 4200-4500) - self-committing seal
...
Say "RADIATION patch 4600 - Binding (gate: 4100 OK - supersedes 4200-4500)"
...
if ((Get-ChildItem "evidence/tasks/TID-2026-09-14-j/commands" | Measure-Object).Count -ne 5) { Die "TID-j bundle commands incomplete" }
Say "OK archive (15) + canonical bundles (h, i, j) present"
...
if ((Get-Content "README.md" -Raw) -notmatch "v2\.9\.0") { Die "version not v2.9.0" }
...
if ($LASTEXITCODE -ne 0) { Die "relay self-test failed (want 11/11)" }
...
if (git rev-parse --is-inside-work-tree 2>$null) {
  git add -A
  $staged = git diff --cached --quiet; if ($LASTEXITCODE -eq 0) {
    Say "OK tree already sealed - nothing to commit"
  } else {
    git commit -q -m "Seal 4600 binding patch"
    if ($LASTEXITCODE -eq 0) { Say "OK AUTO-COMMITTED the sealed tree" }
    else { Say "X commit failed (git identity?) - seal manually" }
  }
}
Say "  -> PUSH (the Commander's motor act): git push"
Say "  -> PROVE IT: fresh clone -> 34 checks - 33 pass - 1 warn - 0 FAIL"
