#!/usr/bin/env bash
# APPLY.sh — RADIATION patch 4500 "Last Mile" (SUPERSET of 4200+4300+4400)
# Seal transaction: after this, COMMIT + PUSH is mandatory (F-01 lesson, four times now).
set -euo pipefail
say(){ printf '%s\n' "$*"; }; die(){ printf '✗ %s\n' "$*" >&2; exit 1; }
...
say "☢️  RADIATION patch 4500 — Last Mile (gate: 4100 ✓ · supersedes 4200+4300+4400)"
...
[ "$(ls evidence/tasks/TID-2026-09-14-i/commands/ | wc -l)" -eq 5 ] || die "TID-i bundle commands incomplete"
say "     ✓ archive (15) + canonical bundles (h, i) present"
...
grep -q "v2.8.0" README.md || die "version not v2.8.0"
grep -q "II.10 — LEDGER COMPRESSION" docs/AI_RULES.md || die "II.10 absent"
grep -q "CLOSED" docs/AI_RULES.md || die "II.10.6 closed-list clause absent"
python3 -m radiation_core.relay --self-test >/dev/null 2>&1 || die "relay self-test failed (want 6/6)"
...
say "  Then prove it: fresh clone -> validate.py = 34 checks · 33 pass · 1 warn · 0 FAIL"
say "  CI is now BLOCKING on the apply report — a red seal stops the merge."
say "═══════════════════════════════════════════════════════════════════════════"
