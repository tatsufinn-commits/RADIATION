...
```
- `validate.py` — 26 structural checks (form + resolvability, never truth). Exit 1 = at least one ❌ FAIL → the session is NOT closable; fix or report the failure honestly. ⚠️ WARN entries are ratification-pending items — list them in your delivery, do not "fix" canon to silence them.
...
`decay_compute.py` · `ics_normalize.py`.
...
That guard exists because this file said "14 checks" for as long as the validator ran 25.
