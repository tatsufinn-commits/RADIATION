...
```
- `validate.py` — the structural validator (form + resolvability, never truth; the live check count is printed by every run — never trust a number written in a file). Exit 1 = at least one ❌ FAIL → the session is NOT closable; fix or report the failure honestly. ⚠️ WARN entries are ratification-pending items — list them in your delivery, do not "fix" canon to silence them.
