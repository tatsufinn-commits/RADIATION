...
| `scripts/contract_tests.py` | `python3 scripts/contract_tests.py` | E1/E2 negative fixtures: declared course-corpus + replica contracts (5300) — undeclared/drift/escape shapes must fail, sanctioned shapes pass. Zero-write (temp trees). |
...
| Provider activation (routing + RADIATION PASS handoff) | our `agents/` layer + `agents/_common/radiation_pass.py` (check 38) | review E4 minimum shape: known-host routing, neutral fallback, zero-write pass, no elevation | [I] | 5200 |
