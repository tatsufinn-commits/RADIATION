...
**Last updated:** 2026-09-13 · **Version:** v2.5.0
...
## REPOSITORY VERSION
**RADIATION v2.5.0** — the Commander's expansion proposals SHIPPED: @Fetch (the retrieval strategist over OPEN_SOURCES, his 50-category bank) + @Overule (the deadline gear, Commander-triggered, AI-flagged rules only) + subskills codified as part of mode autonomy (🟢/🟡/🔴 invocation) + the universal matrix row RESTORED and PINNED (check 28) + the 4000 content gap remediated. Previously: v2.4.2 — recalibrated by the Commander in person: capability first, content on order; the 3500 cue lessons finally in the repo (extraction gap found + remediated); the two new facts are machine-locked. Previously: v2.4.1 — two cards in the Core: CARD_002 defuses a live stale-generation trap (PD 1308 cited as current; RA 10587 repealed it in 2013 — registers corrected, reviewer annotated, curriculum posture held). Previously: v2.4.0 — the relay is machine-checked (check 27: every task chain complete or it FAILs) and the injection surface has a rehearsed defense (tabletop; live drill proposed, ask-gated). Phase-0 closed: 4 real chains, behavior verified. Previously: v2.3.1 — the Core has radiated: CARD_001 (BP 344) admitted via 3 independent channels; 02-analyze exercised; the 1:12/1:20 generation trap locked to KR-LAW-005. Previously: v2.3.0 — self-directed work is now CURATED work: proc_self-directive v1.1 (default-deny + classifier triad, the curation gate, reflection triggers, rollback closure, draw quota) + SD-GOV-012 + the compression ladder (episode→cue→directive promotion + verified-slowest decay). Previously: v2.2.0 — the Autopilot pipeline is INSPECTABLE: the neuron relay (`scaffolding/neurons/`, research-applied closed-loop design) records every 🟡 build at stage granularity; `proc_self-directive.md` operationalizes @selfdirectives; WAYFINDING routes the lost; TOOLBOX grades the rescue kit; the vehicle-resurrection loop is dead (untrack). Previously: v2.1.0 — enforcement sweep complete: apply state is AUDITABLE (verify_apply.py + CI non-blocking apply-report), the vehicle rule is generic, the shrine mandate (AI_RULES II.9) is law with teeth (check 26), and a 3200 payload omission (the A54 scrubber fix) was caught by regenerating and HEALED. Previously: v2.0.0 — applied governance: the Commander's research memo is IN the
